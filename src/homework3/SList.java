...
public class SList {
	...
	public SListNode nthnode(int position) {
		SListNode currentNode;
	...
	public void squish() {
		for(int i=1; i< this.size; i++){
			if(this.nth(i).equals(this.nth(i-1))){
				this.nthnode(i-1).next = this.nthnode(i+1); 
				size -= 1;
			}  
		}
	}

	public void copy(SList list) throws NullPointerException{
		if(list.size != this.size){
			throw new NullPointerException();
		}
		SList output = new SList();
		for(int i = 1; i<list.size+1; i++){
			output.insertEnd(list.nthnode(i));
			//nthnode(i) = list.nthnode(i);
		}
	...
	public boolean contains(Object term){
		if(this.head.next != null){
			...
			for(int i = 0 ; i < this.size; i++){
				if(node.item.equals(term)){
					return true;
				}
				node = node.next;
			}
		}else{
			if(head.item.equals(term)){
				return true;
			}
		}
		return false;
	...
	public void twin() {
		SListNode tester =  this.nthnode(0);
		SListNode copy;
		for(int i = 0; i<this.size; i++){
			copy = new SListNode(tester.item, tester.next);
			tester.next = copy;
			tester = copy.next;
		}
	}
