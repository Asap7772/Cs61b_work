...
public class Homework3 {
		...
		/* Stopped Working :-(
			for(int i = 0; i< ints.length; i++){
				if(ints[i] == -1){
					...
					if(numOf(ints, -1)>0){
						ints[First(ints, -1)] = ints[ints.length-1];
	...
	public static String copyArr(int[] arr1, int[] arr2){
	...
	public static int numOf(int[] arr, int term){
		int num = 0;
	...
	public static int First(int[] arr, int term){
		for(int i = 0; i < arr.length; i++){
			if(arr[i] == term){return i;}
		}
		return -1;
	}
	...
	public static void setAll(int arr[], int term){
		for(int i =0; i< arr.length; i++){
			arr[i] = term;
		}		  
	}

	public static boolean contains(int[] arr, int term){
		for(int i = 0; i < arr.length; i++){
			if(arr[i] == term){
				return true;
			} 
